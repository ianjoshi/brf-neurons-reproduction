...
# [MOD] New function for linear decay in RF
def rf_update_linear(x, u, v, b, omega, dt: float, theta: float =DEFAULT_RF_THETA):
    print("linear decay")
    u_ = u - b * dt - omega * v * dt + x * dt
    v = v + omega * u * dt - b * dt
    z = functional.FGI_DGaussian(u_ - theta)

    return z, u_, v

# [MOD] New function for linear decay in BRF
def brf_update_linear(x, u, v, q, b, omega, dt: float, theta: float =DEFAULT_RF_THETA):
    u_ = u - b * dt - omega * v * dt + x * dt
    v = v + omega * u * dt - b * dt
...
class RFCell(torch.nn.Module):
    ...
    # def reset_trace(self):  # [MOD] Clear traces
    #     self.u_trace = []
    #     self.v_trace = []
    #     self.q_trace = []
    #     self.z_trace = []
    ...
    def forward(
        ...
        # [MOD] Choose update function based on linear decay flag
        if self.use_linear_decay:
            z, u, v = rf_update_linear(  # [MOD]
                x=in_sum, u=u, v=v, b=b, omega=omega, dt=self.dt)  # [MOD]
        else:
            z, u, v = rf_update(
                x=in_sum, u=u, v=v, b=b, omega=omega, dt=self.dt)

        # self.u_trace.append(u.detach().cpu()) # [MOD]
        # self.v_trace.append(v.detach().cpu()) # [MOD]
        # self.z_trace.append(z.detach().cpu()) # [MOD]
...
class BRFCell(RFCell):
    def forward(
        ...
        # [MOD] Choose update function based on linear decay flag
        if self.use_linear_decay:
            z, u, v, q = brf_update_linear(  # [MOD]
                x=in_sum, u=u, v=v, q=q, b=b, omega=omega, dt=self.dt)  # [MOD]
        else:
            z, u, v, q = brf_update(
                x=in_sum, u=u, v=v, q=q, b=b, omega=omega, dt=self.dt)

        # self.u_trace.append(u.detach().cpu())  # [MOD]
        # self.q_trace.append(q.detach().cpu())  # [MOD]
        # self.v_trace.append(v.detach().cpu())  # [MOD]
        # self.z_trace.append(z.detach().cpu())  # [MOD]
